...
{ // Auras by Functor
	...
	if (!message || !*message || sscanf(message, "%d %d %d", &aura_part_1, &aura_part_2, &aura_part_3) < 3)
	{
...
{ // Auras by Functor
	...
	if (!message || !*message || (show_auras_mode = atoi(message)) < 0 || show_auras_mode > 2)
	{
...
ACMD_FUNC(colornick)
{
	...
	if (!message || !*message || sscanf(message, "%d", &group_id) < 1)
	{
