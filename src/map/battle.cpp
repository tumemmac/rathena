...
static int battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case MA_SHARPSHOOTING:
#ifdef RENEWAL
			skillratio += 50 + (200 * skill_lv) + 650;
			RE_LVL_DMOD(100);
#else
		...
		case NPC_ARROWSTORM:
			...
			if (sc && sc->data[SC_FEARBREEZE])
				skillratio += skillratio + 700;
			break;
		case RA_AIMEDBOLT:
			skillratio += 100 + 20 * skill_lv + 500;
			RE_LVL_DMOD(100);
			if (sc && sc->data[SC_FEARBREEZE])
				skillratio += skillratio + 250;
			break;
		case RA_CLUSTERBOMB:
			skillratio += 100 + 100 * skill_lv;
			break;
		...
		case KO_JYUMONJIKIRI:
			...
			break;
		case KO_HUUMARANKA:
			skillratio += -100 + 150 * skill_lv * (status_get_lv(src) / 100) + sstatus->str + (sd ? pc_checkskill(sd,NJ_HUUMA) * 100 : 0);
			RE_LVL_DMOD(100);
			if (sc && sc->data[SC_KAGEMUSYA])
				skillratio += skillratio * sc->data[SC_KAGEMUSYA]->val2 / 100;
			break;
